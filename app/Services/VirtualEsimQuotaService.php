...
class VirtualEsimQuotaService
{
    ...
    public function allowsPaidTopupWhileSuspended(Simcard $simcard): bool
    {
        ...
        $state = strtoupper(trim((string) data_get($simcard->virtual_fulfillment_recipe, 'quota.state', '')));

        return in_array($state, ['SUSPENDED', 'SUSPEND_QUEUED', 'SUSPENDING', 'SUSPEND_RETRYING', 'RESTORE_REQUIRED'], true)
    ...
    public function restoreForPaidTopup(Simcard $simcard): void
    {
        ...
        if ($iccid === null) {
            throw new RuntimeException('Quota-capped eSIM ICCID is not ready for reactivation.', 503);
        }
        ...
        if (! $this->providerMutationSucceeded($response)) {
            ...
            if (strtoupper(trim((string) ($provider['esimStatus'] ?? ''))) !== 'IN_USE') {
                throw new RuntimeException('Provider could not reactivate the quota-capped eSIM for the paid top-up.', 503);
            }
        }
    ...
    public function processPending(int $limit = 100, ?string $onlySimcardId = null, bool $force = false): array
    {
        ...
        $candidates = $query->limit(5000)->get()
            ...
            ->filter(function (Simcard $simcard) use ($force): bool {
                ...
                try {
                    return \Illuminate\Support\Carbon::parse($last)->diffInSeconds(now(), true) >= self::CHECK_INTERVAL_SECONDS;
                } catch (Throwable) {
                    return true;
                }
            })
