...
class EnforceVirtualEsimDurationJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    {
        return [
            // Share the same lock as byte-quota enforcement so one profile never
            // receives concurrent provider suspend/restore mutations.
    ...
    {
        try {
            $service->enforceDurationSuspend($this->simcardId);
        } catch (Throwable $exception) {
            $service->markDurationSuspendRetrying($this->simcardId, $exception->getMessage());
            throw $exception;
        }
    }
    ...
    {
        try {
            app(VirtualEsimQuotaService::class)->markDurationSuspendRetrying(
                $this->simcardId,
                $exception?->getMessage() ?? 'Virtual eSIM duration suspension exhausted its retries.',
            );
        } catch (Throwable) {
